<%--
  Created by IntelliJ IDEA.
  User: 周涵
  Date: 2019/10/23
  Time: 16:54
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <link rel = "stylesheet" type="text/css" href="../../css/Eme.css">
    <title>应急预案信息管理</title>
</head>

<body>

<div class = "header">
    <h1>应急预案信息管理</h1>
</div>

<div class ="nav">
    <a href="index.html">主&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp页</a><br>
    <a href="alethistory.cgi">应急预案</a><br>
    <a href="changt.html">增加应急预案</a><br>
    <a href="changh.html">删除应急预案</a><br>
    <a href="changc.html">查询物资管理</a><br>
    <a href="changc.html">修改演练管理</a><br>
</div>

<div class ="section">
    <h1>欢迎hhhhhhhhhhhhhhhhhhhhh</h1>
</div>

</body>
</html>
